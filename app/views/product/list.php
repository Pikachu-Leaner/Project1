<div class="d-flex flex-column flex-md-row justify-content-between align-items-center mb-4 bg-white p-3 rounded shadow-sm border border-light gap-3">
    ...
    <div class="d-flex align-items-center gap-3">
        <div class="d-flex align-items-center">
            ...
            $currCat = isset($_GET['category']) ? $_GET['category'] : '';
            $currSort = isset($_GET['sort']) ? $_GET['sort'] : 'noi_bat';
            $baseParams = "?brand=" . urlencode($currBrand) . "&category=" . urlencode($currCat) . "&sort=";
            ?>
            <select class="form-select form-select-sm shadow-sm border-secondary" style="min-width: 150px; font-weight: 500;" 
                    onchange="window.location.href='<?= BASE_URL ?>Product/list<?= $baseParams ?>' + this.value + '#shop-section'">
        ...
        <div class="list-group shadow-sm">
            ...
            <?php if(!empty($categories)): ?>
                <?php foreach ($categories as $cat): ?>
                    <a href="<?= BASE_URL ?>Product/list?category=<?= $cat['id'] ?>&sort=<?= $currSort ?>&brand=<?= urlencode($currBrand) ?>#shop-section" 
                       class="list-group-item list-group-item-action fw-bold <?= (isset($_GET['category']) && $_GET['category'] == $cat['id']) ? 'active' : '' ?>">
                    ...
                    <div class="col">
                        <div class="card h-100 shadow-sm border-light position-relative product-card">
                            ...
                            <div class="card-body d-flex flex-column">
                                <h6 class="card-title text-truncate fw-bold mb-3"><?= htmlspecialchars($product['name']) ?></h6>
                                ...
                                <div class="d-flex gap-2 pt-3 border-top mt-auto">
                                    <a href="<?= BASE_URL ?>Cart/add/<?= $product['id'] ?>" class="btn btn-outline-primary btn-sm flex-fill" title="Thêm vào giỏ hàng"><i class="fas fa-cart-plus"></i></a>
                                    <a href="<?= BASE_URL ?>Product/edit/<?= $product['id'] ?>" class="btn btn-outline-secondary btn-sm flex-fill" title="Sửa"><i class="fas fa-edit"></i></a>
                                    <button type="button" class="btn btn-outline-danger btn-sm flex-fill" data-bs-toggle="modal" data-bs-target="#deleteModal" data-id="<?= $product['id'] ?>" data-name="<?= htmlspecialchars($product['name']) ?>" title="Xóa"><i class="fas fa-trash"></i></button>
                                </div>
                            </div>
                        </div>
                    </div>
...
<div class="modal fade" id="deleteModal" tabindex="-1" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content border-0 shadow-lg">
      <div class="modal-header bg-danger text-white"><h5 class="modal-title fw-bold">Xác nhận xóa</h5><button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button></div>
      <div class="modal-body py-4 text-center"><p>Bạn muốn xóa sản phẩm này?</p><p class="fw-bold text-danger fs-5" id="deleteProductName"></p></div>
